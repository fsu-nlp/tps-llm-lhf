...
Workflow:
    - Input: JSONL files with cleaned text (e.g., from scripts/clean_gpt.py)
    - Output: CoNLL-U files with POS/morph/lemma/deps/NER
    - Folder structure is preserved from input to output
    - Read the JSONL file into a list of texts + lightweight metadata (idx, prompt_id, model_name, source path)
    - Build a stream of (text, meta) tuples and feed to nlp.pipe with your --batch-size and --n-process
    - Write CoNLL-U to output file with doc_id based on source filename + idx
...
If a transformer is present: keep n_process=1
"""
...
def print_system_info():
    ...
    print("[INFO] SYSTEM & COMPUTATIONAL SETUP", file=sys.stderr)
    print("=" * 60, file=sys.stderr)

    # --- Basic system info ---
    print(f"[INFO] Platform: {platform.system()} {platform.release()} ({platform.version()})", file=sys.stderr)
    print(f"[INFO] Python version: {platform.python_version()}", file=sys.stderr)
    print(f"[INFO] Machine: {platform.machine()}", file=sys.stderr)

    # --- CPU information (robust detection) ---
    try:
        cpu_name = platform.processor()

        # If platform.processor() is generic, try Linux-specific methods
        if not cpu_name or cpu_name.lower() in ("x86_64", "amd64", "arm64"):
            try:
                # Try lscpu (best for Linux)
        ...
        print(f"[INFO] CPU: {cpu_name}", file=sys.stderr)
        print(f"[INFO] CPU cores: {cpu_count_physical} physical / {cpu_count_logical} logical", file=sys.stderr)
        cpu_freq = psutil.cpu_freq()
        if cpu_freq:
            print(f"[INFO] CPU frequency: {cpu_freq.current:.1f} MHz", file=sys.stderr)

    except Exception as e:
        print(f"[WARN] Unable to read CPU info: {e}", file=sys.stderr)
    ...
    try:
        ...
        print(f"[INFO] System memory: {total_gb:.2f} GB total | {avail_gb:.2f} GB available", file=sys.stderr)
    except Exception as e:
        print(f"[WARN] Unable to read memory info: {e}", file=sys.stderr)
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            print("[INFO] GPU(s) detected:", file=sys.stderr)
            for line in result.stdout.strip().splitlines():
                name, total, used = [p.strip() for p in line.split(",")]
                print(f"    Name: {name}", file=sys.stderr)
                print(f"    Memory total: {total} MiB | Memory used: {used} MiB", file=sys.stderr)
        else:
            print("[WARN] No NVIDIA GPU detected or nvidia-smi unavailable.", file=sys.stderr)
    except FileNotFoundError:
        print("[WARN] nvidia-smi not found. Skipping GPU details.", file=sys.stderr)

    # --- Package versions ---
    print("\n[INFO] Installed key packages:", file=sys.stderr)
    key_pkgs = ["spacy", "torch", "cupy", "numpy", "pandas", "scipy", "transformers"]
    for pkg in key_pkgs:
        try:

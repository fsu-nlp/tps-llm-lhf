...
def load_spacy_model(name: str, posonly: bool, use_gpu: bool = True):
    ...
    disable: List[str] = ["parser", "ner"] if posonly else []

    # ✅ Ask for GPU BEFORE loading a transformer model
    ...
    # Load the pipeline (now it will initialize on CUDA if available)
    try:
    ...
    # Ensure we still get sentence boundaries if parser/senter missing
    if "parser" not in nlp.pipe_names and "senter" not in nlp.pipe_names:
        nlp.add_pipe("sentencizer")

    # Optional: light warmup if transformer present and GPU requested
